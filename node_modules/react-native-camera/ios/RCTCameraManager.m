...
- (NSDictionary *)constantsToExport
{
  return @{
           ...
           @"videoWidthForCrop": @400,
           @"videoHeightForCrop": @400
           };
}
...
RCT_EXPORT_VIEW_PROPERTY(videoWidthForCrop, NSInteger)
RCT_EXPORT_VIEW_PROPERTY(videoHeightForCrop, NSInteger)
                ...
                error:(NSError *)error
{
  ...
  if (self.videoTarget == RCTCameraCaptureTargetCameraRoll) {
    ...
    if ([library videoAtPathIsCompatibleWithSavedPhotosAlbum:outputFileURL]) {
      ...
      [self cropVideoAtPath:[NSURL fileURLWithPath:fullPath] withVideoInfo:videoInfo];
...
/*---------------VIDEO CROP CODE Start---------------*/

-(void)cropVideoAtPath:(NSURL*)filePath withVideoInfo:(NSMutableDictionary*)videoInfo{
    // output file
    ...
    NSURL *outputPathURL = [NSURL URLWithString:outputPath];
    // input file
    AVURLAsset* videoAsAsset = [AVURLAsset URLAssetWithURL:filePath options:nil];
    
    AVMutableComposition *composition = [AVMutableComposition composition];
    [composition  addMutableTrackWithMediaType:AVMediaTypeVideo preferredTrackID:kCMPersistentTrackID_Invalid];
    
    // input clip
    AVAssetTrack* clipVideoTrack = [[videoAsAsset tracksWithMediaType:AVMediaTypeVideo] objectAtIndex:0];
    
    // crop clip to screen ratio
    UIInterfaceOrientation orientation = [self orientationForTrack:videoAsAsset];
    BOOL isPortrait = (orientation == UIInterfaceOrientationPortrait || orientation == UIInterfaceOrientationPortraitUpsideDown) ? YES: NO;
    CGFloat complimentSize = [self getComplimentSize:clipVideoTrack.naturalSize.height];
    CGSize videoSize;
    if(isPortrait) {
        videoSize = CGSizeMake(clipVideoTrack.naturalSize.height, complimentSize);
    } else {
        videoSize = CGSizeMake(complimentSize, clipVideoTrack.naturalSize.height);
    }
    
    AVMutableVideoComposition* videoComposition = [AVMutableVideoComposition videoComposition];
    videoComposition.frameDuration = CMTimeMake(1, 30);
    
    videoComposition.renderSize = videoSize;//CGSizeMake(clipVideoTrack.naturalSize.height, clipVideoTrack.naturalSize.height);
    NSLog(@"size is here now%f,%d,%d,%f",clipVideoTrack.naturalSize.height,self.videoHeightForCrop,self.videoWidthForCrop,clipVideoTrack.naturalSize.width);
    ...
    CGAffineTransform t4 = CGAffineTransformMakeTranslation(clipVideoTrack.naturalSize.height, -(clipVideoTrack.naturalSize.width - complimentSize) /2 );
    CGAffineTransform t5 = CGAffineTransformRotate(t4, M_PI_2);

    [transformer setTransform:t5 atTime:kCMTimeZero];
    instruction.layerInstructions = [NSArray arrayWithObject: transformer];
    videoComposition.instructions = [NSArray arrayWithObject: instruction];

    
    // export
    AVAssetExportSession *exporter = [[AVAssetExportSession alloc] initWithAsset:videoAsAsset presetName:AVAssetExportPresetHighestQuality] ;
    ...
    [exporter exportAsynchronouslyWithCompletionHandler:^(void){
        ...
        ALAssetsLibrary *library = [[ALAssetsLibrary alloc] init];
        if ([library videoAtPathIsCompatibleWithSavedPhotosAlbum:outputPathURL]) {
...
- (CGFloat)getComplimentSize:(CGFloat)size {
    CGFloat ratio = (float)((float)self.videoHeightForCrop / (float)self.videoWidthForCrop);
    return size * ratio;
}

- (UIInterfaceOrientation)orientationForTrack:(AVURLAsset *)asset {
